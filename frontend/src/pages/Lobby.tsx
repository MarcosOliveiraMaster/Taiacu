    ...
    <div className="min-h-screen bg-gray-950 flex flex-col px-5 py-8 max-w-md mx-auto">
      ...
      <div className="flex bg-gray-800 rounded-2xl p-1 gap-1 mb-6">
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${
            modo === 'criar' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'
          }`}
        >
          🏠 Criar Sala
        </button>
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${
            modo === 'entrar' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'
          }`}
        >
          🎮 Entrar
        </button>
      ...
      {modo === 'criar' && (
        <form onSubmit={handleCriar} className="flex flex-col gap-5">
          <div>
            <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1.5 block">Nome da sala</label>
            <input
              type="text" placeholder="Ex: Pagode do Zé"
          ...
          <div>
            <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-3 block">Modo de Jogo</label>
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setModoJogo('sussegado')}
                className={`p-4 rounded-2xl border-2 transition-all text-left ${
                  modoJogo === 'sussegado' ? 'border-purple-500 bg-purple-900/40 shadow-lg shadow-purple-900/30' : 'border-gray-700 bg-gray-900'
                }`}>
                <p className="text-2xl mb-1">😌</p>
                <p className="text-white font-black text-sm">Sussegado</p>
                <p className="text-gray-400 text-xs mt-1 leading-relaxed">Acertou: +1pt<br/>Não descoberto: +3pts</p>
              </button>
              <button type="button" onClick={() => setModoJogo('arretado')}
                className={`p-4 rounded-2xl border-2 transition-all text-left ${
                  modoJogo === 'arretado' ? 'border-orange-500 bg-orange-900/20 shadow-lg shadow-orange-900/20' : 'border-gray-700 bg-gray-900'
                }`}>
                <p className="text-2xl mb-1">🔥</p>
                <p className="text-white font-black text-sm">Arretado</p>
                <p className="text-gray-400 text-xs mt-1 leading-relaxed">Acertou: 30/tempo<br/>Não descoberto: +40pts</p>
              </button>
            </div>
          </div>
          ...
          <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800 flex flex-col gap-5">
            <div>
              ...
              <input type="range" min={2} max={10} value={maxJogadores}
            ...
            <div className="border-t border-gray-800 pt-5">
              ...
              <input type="range" min={1} max={5} value={musicasPorJogador}
      ...
      {modo === 'entrar' && (
        <form onSubmit={handleEntrar} className="flex flex-col gap-5">
          <div>
            <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider mb-1.5 block">Código da sala</label>
            <input
          ...
          <button type="submit" disabled={carregando || codigo.length < 4}
